# 🧩 Requirements: Sales Report (Báo cáo Doanh số)

> **📋 STATUS: ✅ IMPLEMENTED** - Modern report with aggregated summary & lazy-loaded details  
> **🔗 Implementation**: `src/features/reports/sales/`  
> **🔧 Last Updated**: 2025-01-18 - Summary table improvements (no pagination, totals footer), source mapping, view renamed
> **📱 Route**: `/reports/sales`
...
Sales Report (Báo cáo Doanh số) là trang phân tích tổng quan cho giá trị dịch vụ đã tư vấn (bao gồm cả "Đã chốt" và "Chưa chốt"), với:

### Core Features (✅ Đã triển khai đầy đủ)

1. **🔍 Page Header with Navigation**:

   - **Component**: `PageHeaderWithMonthNav` (shared component)
   - Month picker (chọn tháng phân tích)
   - Clinic selector (admin only, hiển thị tên chi nhánh)
   - Auto-label: "Tháng này", "Tháng trước", hoặc "Tháng MM/YYYY"
   - Responsive layout: Title bên trái, controls bên phải

2. **📊 Sales Report Stats** (4 cards):

   - Tổng doanh số với 2 growth indicators (MoM + YoY)
   - Số dịch vụ chốt với 2 growth indicators
   - Số khách mới với 2 growth indicators
   - Khách mới vs Cũ (revenue breakdown)

3. **📊 Summary Tabs** (5 tabs - tổng hợp):

   - Tab 1: **Theo ngày** (daily breakdown - sorted by date, ranked by revenue)
   - Tab 2: **Theo nguồn** (source analysis)
   - Tab 3: **Theo dịch vụ** (service category)
   - Tab 4: **Theo sale** (sale performance with ranking)
   - Tab 5: **Theo bác sĩ tư vấn** (doctor consultation)
   - **UI**: Default line tabs (không dùng type="card")
   - **Table styling**: `size="small"`, `bordered` (consistent with project pattern)
   - **Pagination**: Không phân trang (hiển thị tất cả records)
   - **Summary Row**: Hiển thị tổng doanh số và các metrics ở cuối bảng
   - **Reusable Component**: Tất cả tabs dùng chung `SummaryTable` component để giảm duplication

4. **📋 Detail Panel** (always visible):

   - Hiển thị lúc nào cũng có (skeleton/empty state khi chưa chọn row)
   - Danh sách ConsultedService chi tiết khi click row trong Summary Tabs
   - **8 fixed columns**: Ngày tư vấn, Ngày chốt, Dịch vụ, Khách hàng (clickable link), Nguồn khách, Sale tư vấn, Bác sĩ tư vấn, Giá trị, Trạng thái
   - **Table styling**: `size="small"`, `bordered` (consistent with project)
   - Component dùng chung cho tất cả tabs
...
- Tất cả users có quyền truy cập `/reports/sales`
- **Admin**: Có thể chọn clinic trong filter để xem dữ liệu của bất kỳ chi nhánh nào (hoặc tất cả)
- **Employee**: Chỉ xem được dữ liệu của clinic mình thuộc về (clinic filter không hiển thị)

**Dữ liệu hiển thị**:

- **Summary Tabs**: Hiển thị dữ liệu theo `consultationDate` (ngày tư vấn) - bao gồm cả dịch vụ đã chốt và chưa chốt
- **Detail Panel**: Hiển thị tất cả dịch vụ (không filter theo serviceStatus), có cột "Ngày chốt" (nullable) và "Trạng thái"
...
- ✅ **Single Page Dashboard**: Không dùng tabs cho clinics, dùng Select filter thay vì
- ✅ **PageHeaderWithMonthNav Component**: Tạo shared component tương tự PageHeaderWithDateNav
  - Title bên trái với auto-label (Tháng này, Tháng trước, Tháng MM/YYYY)
  - Month picker và Clinic selector bên phải
  - Responsive layout với Row/Col
  - Reusable cho các report features khác
- ✅ **Ant Design 5.x**: Sử dụng modern API
  - `Card` với `variant="borderless"` (thay vì `bordered={false}`)
  - `Tabs` với `items` prop và default line style (không dùng `type="card"`)
  - Consistent với pattern của ClinicTabs và CustomerDetailView
- ✅ **Table Styling Consistency**:
  - `size="small"` cho tất cả tables (SummaryTable và DetailPanel)
  - `bordered` để chia dòng rõ ràng, dễ đọc
  - Pattern giống TreatmentLogTable, EmployeeTable
- ✅ **Component Naming**:
  - `KpiCards` → `SalesReportStats` (consistent với `EmployeeStats`)
  - File renamed: `KpiCards.tsx` → `SalesReportStats.tsx`
  - Export updated trong barrel export
- ✅ **Layout**: Card-based với shadow và border radius
  - Không có background color riêng (dùng AppLayout background)
- ✅ **KPI Cards**: 4 cards với 2 growth indicators mỗi card (MoM + YoY)
  - Color-coded: Blue (sales), Green (deals), Purple (new customers), Mixed (customer type)
  - Growth arrow: Up (green) / Down (red) vs tháng trước
- ✅ **Responsive Grid**:
  - KPI Cards: 4 columns (lg), 2 columns (sm), 1 column (xs)
  - Mobile: Stack vertically
- ✅ **Code Optimization**: Created reusable `SummaryTable` component
  - Reduced SummaryTabs from 516 lines → 93 lines (85% reduction)
  - Generic component với configurable nameColumn prop
- ✅ **DetailPanel**: Always visible với empty state
  - Skeleton/guidance when no row selected
  - Fixed 8 columns across all tabs (không còn dynamic columns)
  - Customer name clickable → navigate to `/customers/:id`
  - Table với `size="small"` và `bordered`
  - **Export button removed** từ DetailPanel (sẽ implement tập trung sau)

### Data & Business Logic (✅ Implemented)

- ✅ **Data Source**: `ConsultedService` KHÔNG filter theo serviceStatus
  - Tab Ngày: Filter theo `consultationDate` (ngày tư vấn) - bao gồm cả chưa chốt
  - Detail Panel: Hiển thị tất cả services (có cột Trạng thái để phân biệt)
- ✅ **Date Fields Clarification**:
  - `consultationDate`: Ngày tư vấn (required) - dùng để filter data
  - `serviceConfirmDate`: Ngày chốt (nullable) - chỉ có khi status = "Đã chốt"
- ✅ **Column Names**: "Ngày chốt" (clarified from "Ngày") để rõ ràng
- ✅ **Daily Tab Sorting**: Sắp xếp theo ngày ASC, nhưng ranking theo revenue DESC
  - Ranking calculation: Sort by revenue → assign rank → re-sort by date
  - Display: Dates chronological, ranks revenue-based
- ✅ **Timezone Fix**: Date parsing với `new Date(year, month-1, day)` thay vì string parsing
- ✅ **Customer Classification**: Phân loại khách mới/cũ dựa vào `Customer.createdAt`
  - **Khách mới**: `createdAt` trong tháng đang xem
  - **Khách cũ**: `createdAt` trước tháng đang xem
  - Doanh số tính từ `ConsultedService.finalPrice`, group by customer classification
- ✅ **Clinic Filter**: Admin sees clinic names (fetched via `useClinics` hook) with "Tất cả chi nhánh" option
- ✅ **Clear on Filter Change**: Selected row cleared when month/clinic filter changes to prevent stale data

### Component Architecture (✅ Implemented)

- ✅ **Feature-based structure**: `src/features/reports/sales/`
...
- ✅ **Type safety**: Full TypeScript types in `shared/validation/sales-report.schema.ts`
- ✅ **Reusable**: Các component độc lập, exported trong barrel export
- ✅ **React Query**: Dynamic stale time (2 min current month, 2 hours past)
- ✅ **Repository Layer**: Uses Zod types from shared/validation for consistency

---

## 1. 🎨 UI Components (Đã triển khai)

### 1.1 Layout Structure

```
┌──────────────────────────────────────────────────────────┐
│  AppLayout (trắng background)                            │
│  ┌────────────────────────────────────────────────────┐  │
│  │ ReportFilters Card (white, shadow)                 │  │
│  │ ┌──────────────────────────────┬─────────────────┐ │  │
│  │ │ [Month] [Clinic]             │ [Excel]         │ │  │
│  │ └──────────────────────────────┴─────────────────┘ │  │
│  └────────────────────────────────────────────────────┘  │
│                                                          │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐        │
│  │ Tổng DS │ │ DV chốt │ │ KH mới  │ │ Mới/Cũ  │ (KPI) │
│  │ 1.25B ₫ │ │ 156     │ │ 85      │ │ 680M/   │        │
│  │↑+12.5%  │ │↑+8.3%   │ │↑+15.6%  │ │ 570M    │        │
│  │↑+28.3%  │ │↑+15.2%  │ │↑+22.8%  │ │↑+18.2%  │        │
│  └─────────┘ └─────────┘ └─────────┘ └─────────┘        │
│                                                          │
│  ┌────────────────────────────────────────────────────┐  │
│  │ � Summary Tabs (Tổng hợp - Clickable Rows)        │  │
│  │ [Ngày] [Nguồn] [Dịch vụ] [Sale] [Bác sĩ]           │  │
│  │ ───────────────────────────────────────────────    │  │
│  │ <Table data for active tab - Click row to detail>  │  │
│  │ Tab Sale: Rank | Sale | Assigned | Consulted ...   │  │
│  └────────────────────────────────────────────────────┘  │
│                                                          │
│  ┌────────────────────────────────────────────────────┐  │
│  │ 📋 Detail Panel (Chi tiết - When row selected)     │  │
│  │ ← Quay lại  |  Selected: "01/01/2025"              │  │
│  │ ───────────────────────────────────────────────    │  │
│  │ Danh sách dịch vụ chốt ngày 01/01/2025             │  │
│  │ Customer | Source | Service | Sale | Doctor | $    │  │
...
**File**: `src/shared/components/PageHeaderWithMonthNav.tsx` ✅ **NEW**
...
- **Title with Auto-label**:
  ...
  - Month label auto-generated: "Tháng này", "Tháng trước", hoặc "Tháng MM/YYYY"
...
- **Clinic Selector** (conditional):
  - Select dropdown với clinic names
...
**Layout**: Row với justify-content: space-between

- Left: Title + subtitle
- Right: Clinic Select + Month Picker (Row với gutter 8px)
...
**File**: `src/features/reports/sales/components/SalesReportStats.tsx` ✅ **RENAMED from KpiCards**

**Naming Convention**: Follows pattern of `EmployeeStats` - `{Feature}Stats`

**Layout**: Row với 4 columns responsive
...
1. **Tổng doanh số**:
   ...
   - Value: 1,250,000,000 ₫ (VND format)
   - Color: #1890ff
   - Growth indicators (2 lines):
     - Line 1: +12.5% ↑ (green arrow) - "vs tháng trước"
     - Line 2: +28.3% ↑ (green arrow) - "vs cùng kỳ năm trước"
...
2. **Số dịch vụ chốt**:
   ...
   - Color: #52c41a
   - Growth indicators (2 lines):
     - Line 1: +8.3% ↑ - "vs tháng trước"
     - Line 2: +15.2% ↑ - "vs cùng kỳ năm trước"

3. **Số khách mới**:

   - Icon: UserAddOutlined (purple)
   - Value: 85 (số khách có `Customer.createdAt` trong tháng hiện tại)
   - Color: #722ed1 (purple)
   - Growth indicators (2 lines):
     - Line 1: +15.6% ↑ - "vs tháng trước"
     - Line 2: +22.8% ↑ - "vs cùng kỳ năm trước"

4. **Khách mới vs Cũ**:
   - Dual display (doanh số phân loại theo thời điểm tạo khách hàng):
     - Mới: 680,000,000 ₫ (green, size 16px) - Doanh số từ khách có `Customer.createdAt` trong tháng hiện tại
     - Cũ: 570,000,000 ₫ (blue, size 14px) - Doanh số từ khách có `Customer.createdAt` trước tháng hiện tại
...
**File**: `src/features/reports/sales/components/SummaryTabs.tsx` ✅

**Tabs Type**: Ant Design Tabs với **default line style** (không dùng `type="card"`)

**Styling Consistency**: Matches ClinicTabs và CustomerDetailView pattern

**Key Features**:

- Uses Ant Design 5.x `items` prop (NOT deprecated `TabPane`)
- All tabs use reusable `SummaryTable` component (code reduction: 516→93 lines, 85% savings)
- All tables have `size="small"` and `bordered` props
- All rows clickable để trigger DetailPanel
...
- Standard columns: Ranking | Khách đến | Dịch vụ tư vấn | Dịch vụ chốt | Doanh số | Tỷ lệ chốt % | TB/dịch vụ

**Key Behavior**:

- **Sorting**: Hiển thị theo ngày chronological (ASC)
- **Ranking**: Based on revenue DESC (calculated in backend, không theo date order)
- **Data source**: Filter theo `consultationDate` (ngày tư vấn), bao gồm cả "Chưa chốt"
...
| -------------- | ------ | ------ | ---------------------------------------------------------------------- |
| Ranking        | 110px  | center | Tag with trophy icon (top 3) + % doanh số (small text gray)            |
| [Name]         | varies | left   | Dynamic per tab (date/source/service/sale/doctor), fixed column        |
| Khách đến      | 120px  | center | Number - Số khách                                                      |
| Dịch vụ tư vấn | 110px  | center | Number - Số dịch vụ tư vấn                                             |
| Dịch vụ chốt   | 100px  | center | Green bold number - Số dịch vụ chốt                                    |
| Doanh số       | 140px  | right  | Blue bold, sortable                                                    |
| Tỷ lệ chốt %   | 110px  | center | Progress bar (0-100%), formula: (Dịch vụ chốt / Dịch vụ tư vấn) × 100% |
| TB/dịch vụ     | 120px  | right  | VND compact format - Average revenue per service                       |

---

### 1.5 DetailPanel Component

**File**: `src/features/reports/sales/components/DetailPanel.tsx` ✅

**Purpose**: Hiển thị danh sách chi tiết các ConsultedService records khi user click vào row trong Summary Tabs

**Key Design Decisions**:

- **Always visible**: Component luôn render (không conditional)
- **Empty state**: Hiển thị guidance khi chưa chọn row
- **Fixed columns**: 8 columns cố định cho tất cả tabs (KHÔNG còn dynamic columns)
...
- **Optional props**: All props are optional để support empty state

**Layout**:

```
┌──────────────────────────────────────────────────────┐
│  ← Quay lại  |  Chi tiết ngày: 01/01/2025            │
│  ──────────────────────────────────────────────────  │
│  Tổng số: 15 dịch vụ  |  Tổng doanh số: 125,000,000 ₫ │
│                                                      │
│  <Table: size="small", bordered - 8 fixed columns>  │
└──────────────────────────────────────────────────────┘
```
...
| ------------- | ----- | ----- | --------------------------------------- | ------------------------------------------------ |
| Ngày tư vấn   | 110px | left  | consultationDate (DD/MM/YYYY)           | Required field                                   |
| Ngày chốt     | 110px | -     | serviceConfirmDate (DD/MM/YYYY or "-")  | Nullable - only if status closed                 |
| Dịch vụ       | 200px | -     | dentalService.name                      | -                                                |
| Khách hàng    | 150px | -     | customer.fullName                       | **Clickable Link** to /customers/:id, blue color |
| Nguồn khách   | 130px | -     | customer.source (or "Không rõ")         | -                                                |
| Sale tư vấn   | 140px | -     | consultingSale.fullName (or "-")        | Nullable                                         |
| Bác sĩ tư vấn | 140px | -     | consultingDoctor.fullName (or "-")      | Nullable                                         |
| Giá trị       | 130px | -     | finalPrice (VND format, blue bold)      | Right aligned                                    |
| Trạng thái    | 110px | -     | serviceStatus (Tag: green if "Đã chốt") | -                                                |

**Query Logic** (Backend):

- **Base filter**: `consultationDate` in selected month (KHÔNG filter serviceStatus)
...
- **Back button**: "← Quay lại" để clear selection và quay về Summary Table
- **Header summary**:
  - Dynamic title: "Chi tiết [tab label]: [selected item name]"
...
- **Empty State**:
  ...
  - Message: "Chưa có dữ liệu chi tiết"
  - Guidance: "Vui lòng chọn một dòng trong bảng tổng hợp bên trên để xem chi tiết"
...
- Summary row: Background #fafafa với bold text
- Loading: Ant Design Table built-in loading skeleton

**Card Wrapper**:

- Title: Dynamic based on selected row (e.g., "Chi tiết ngày: 01/01/2025")
...
**File**: `src/features/reports/sales/components/SummaryTable.tsx` ✅
...
**Impact**: Reduced SummaryTabs code from 516 lines → 93 lines (85% reduction)
...
3. **Khách đến**: 120px, center
4. **Dịch vụ tư vấn**: 110px, center
5. **Dịch vụ chốt**: 100px, center, green bold
6. **Doanh số**: 140px, right, blue bold, sortable
7. **Tỷ lệ chốt %**: 110px, center, Progress bar
8. **TB/dịch vụ**: 120px, right, VND compact format

**Summary Row** (Table footer):

- **Display**: Fixed summary row at bottom with bold styling, light gray background
- **Calculations**:
  - Tổng khách đến (sum of customersVisited)
  - Tổng DV tư vấn (sum of consultations)
  - Tổng DV chốt (sum of closed)
  - Tổng doanh số (sum of revenue) - **blue bold highlight**
  - Tỷ lệ chốt % (calculated from totals)
  - TB/dịch vụ (calculated from totals)
...
**File**: `src/features/reports/sales/components/KpiCards.tsx` ✅ (Status: Pending full implementation)
...
1. **Tổng doanh số**:
   ...
   - Color: #1890ff
   - Growth indicators (2 lines):
     - Line 1: totalSalesGrowthMoM % ↑/↓ - "vs tháng trước"
     - Line 2: totalSalesGrowthYoY % ↑/↓ - "vs cùng kỳ năm trước"
...
2. **Số dịch vụ chốt**:
   ...
   - Color: #52c41a
   - Growth indicators (2 lines):
     - Line 1: closedDealsGrowthMoM % ↑/↓
     - Line 2: closedDealsGrowthYoY % ↑/↓
...
3. **Số khách mới**:
   ...
   - Color: #722ed1
   - Growth indicators (2 lines):
     - Line 1: newCustomersGrowthMoM % ↑/↓
     - Line 2: newCustomersGrowthYoY % ↑/↓

4. **Khách mới vs Cũ**:
   - Dual display (revenue breakdown):
     - Mới: newCustomerSales ₫ (green, size 16px)
     - Cũ: oldCustomerSales ₫ (blue, size 14px)
...
## 2. 📋 TypeScript Types (✅ Đã định nghĩa đầy đủ)
...
**Summary Tab Data** (5 types với shared structure):
...
export const ConsultedServiceDetailSchema = z.object({
  id: z.string(),
  consultationDate: z.string(), // ISO date (required) - ngày tư vấn
  serviceConfirmDate: z.string().nullable(), // ISO date (nullable) - ngày chốt
  finalPrice: z.number(),
...
interface SalesSummaryResponse {
  kpi: KpiData; // 9 metrics với growth
  tabs: {
...
- Data source: `ConsultedService` với `serviceStatus = "Đã chốt"` + `serviceConfirmDate` trong month
- Aggregation: GROUP BY date/source/service/employee tùy tab

**Frontend Caching**:

- **Admin**: Query all data → client-side filter by selected clinic (no refetch on clinic change)
- **Employee**: Query scoped data only
- **Stale Time**: Dynamic by month age (2 min current month, 2 hours past months)

### 3.2 Detail API (Chi tiết khi click row)
...
- Base query: `ConsultedService` với `serviceStatus = "Đã chốt"` + `serviceConfirmDate` trong month
...
- **Lazy loading**: Chỉ fetch khi user click row (enabled by queryKey dependency)

**Frontend Caching**:

- **Query Key**: `['sales-dashboard', 'detail', tab, key, month, clinicId]`
...
export function useSalesSummary(filters: DashboardFilters) {
  ...
  const query = useQuery({
    queryKey: ["sales-dashboard", "summary", apiParams],
...
- ✅ Admin query all → client filter (fast clinic switch)
- ✅ Employee query scoped (backend enforced)
- ✅ Dynamic stale time: 2 min (current), 2 hours (past months)
...
export function useSalesDetail(
  ...
  return useQuery({
    queryKey: ["sales-dashboard", "detail", tab, key, month, effectiveClinicId],
    queryFn: () =>
      getSalesDetailApi({
        tab: tab!,
        key: key!,
...
- ✅ Lazy loading với `enabled: !!tab && !!key`
- ✅ Separate cache per tab/key combination
- ✅ Dynamic stale time: 2 min (current), 2 hours (past months)
- ✅ Consistent caching strategy với summary data
...
- [ ] `GET /api/v1/reports/sales/summary` - Role-based clinicId enforcement
  - [x] API Route implementation
  - [x] Service layer with Zod validation
  - [x] Repository layer using Zod types (GetSalesSummaryQuery)
  - [ ] Full integration testing
- [ ] `GET /api/v1/reports/sales/detail` - Tab/key dynamic filtering
  - [x] API Route implementation
  - [x] Service layer with Zod validation
  - [x] Repository layer using Zod types (GetSalesDetailQuery)
  - [ ] Full integration testing
- [ ] Service: Aggregation logic (5 tabs)
  - [x] Daily tab: Sort by revenue for ranking → re-sort by date for display
  - [x] Filter by consultationDate (not serviceConfirmDate)
  - [x] Include both "Đã chốt" and "Chưa chốt" services
  - [ ] Performance optimization for large datasets
- [x] Service: Detail query với full relations (customer, dentalService, consultingSale, consultingDoctor)
- [x] Repo: Date parsing fix (new Date(year, month-1, day))
- [x] Zod schemas validation (✅ Defined in shared/validation/sales-report.schema.ts)
...
8. **Repository Type Safety** (NEW):
   ...
   - `SummaryParams` → `GetSalesSummaryQuery`
   - `DetailParams` → `GetSalesDetailQuery`
...
11. **UI Consistency Improvements** (NEW):
    ...
    - SummaryTabs: Removed `type="card"` → default line tabs (matches ClinicTabs, CustomerDetailView)
    - Tables: Added `size="small"` and `bordered` props (consistent with TreatmentLogTable, EmployeeTable)
    - Component naming: KpiCards → SalesReportStats (consistent with EmployeeStats pattern)
    - DetailPanel: Removed export button (will be centralized later)

12. **View Component Naming** (NEW):

    - Renamed: SalesDashboardView → SalesReportView
    ...
    - File: `SalesDashboardView.tsx` → `SalesReportView.tsx`
...
14. **Customer Source Display Mapping** (NEW):
    ...
    - Example: "facebook" → "Facebook", "tiktok" → "Tiktok"
...
1. **Code Reduction**:
   ...
   - Eliminated 85% duplication (516→93 lines)
...
3. **Memo Opportunities** (if needed):
