...
model LaboOrder {
  ...
  // Dates
  sendDate        DateTime  @db.Date // Ngày gửi mẫu đi xưởng
  returnDate      DateTime? @db.Date // Ngày nhận mẫu về từ xưởng
  expectedFitDate DateTime? @db.Date // Ngày dự kiến lắp cho khách (optional)

  // Details
  detailRequirement String? // Ghi chú yêu cầu chi tiết cho xưởng
  toothPositions    String[] // Mảng vị trí răng: ["16", "26"]
...
- **Date Fields**: `sendDate`, `returnDate`, `expectedFitDate` dùng `@db.Date` (không có giờ, chỉ ngày)
- **receivedById**: Track nhân viên nào xác nhận nhận mẫu (nullable - chỉ set khi nhận)
- **toothPositions**: Array string để lưu vị trí răng cần làm
...
- ✅ **Date Fields**:
  - `sendDate` (required): Ngày gửi mẫu đi xưởng
  - `returnDate` (nullable): Ngày nhận mẫu về - set khi click "Nhận mẫu"
  - `expectedFitDate` (nullable): Ngày dự kiến lắp - optional field
- ✅ **Tracking Fields**:
  - `receivedById`: Track nhân viên xác nhận nhận mẫu (set khi click "Nhận mẫu")
  - `toothPositions`: Array string lưu vị trí răng ["16", "26"]
- ✅ **Daily View Queries**:
