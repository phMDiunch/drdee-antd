...
- [x] Collapsible Table "📤 Mẫu Gửi Đi" hiển thị orders có `sendDate` = date
- [x] Badge đếm số lượng: "Mẫu Gửi Đi (5)"
- [x] Table columns: Khách hàng, Bác sĩ, Ngày điều trị, Loại đơn hàng, Xưởng, Loại răng, SL, Ngày gửi mẫu, Ngày hẹn lắp, Yêu cầu, Thao tác
...
- [x] Collapsible Table "📥 Mẫu Nhận Về" hiển thị orders có `returnDate` = date
- [x] Badge đếm số lượng: "Mẫu Nhận Về (3)"
- [x] Table columns: Tương tự table gửi đi
...
- [x] Admin: Chọn được bất kỳ clinic
...
**Để** gửi yêu cầu làm răng cho khách hàng

**Acceptance Criteria**:

- [x] Button "Tạo đơn hàng" mở CreateLaboOrderModal
- [x] Form fields (Row 1): Khách hàng (_), Bác sĩ (_), Xưởng (_), Loại hàng labo (_)
- [x] Form fields (Row 2): Ngày điều trị (_), Loại đơn hàng (_), Người gửi mẫu (\*)
- [x] Form fields (Row 3): Số lượng (\*), Ngày hẹn lắp, Yêu cầu chi tiết
- [x] Auto-fill pricing: Chọn Xưởng + Loại hàng → Auto load giá và bảo hành từ LaboService
- [x] Auto-calculate: Tổng tiền = Đơn giá × Số lượng (hiển thị read-only)
- [x] Validation: Tất cả required fields, unique constraint (supplierId + laboItemId phải tồn tại trong LaboService)
- [x] Success: Refresh table, toast "Đã tạo đơn hàng labo"
...
- [x] Hiển thị fields (disabled): Khách hàng, Bác sĩ, Xưởng, Loại hàng, Ngày điều trị, Loại đơn hàng, Người gửi
...
- [x] Auto-recalculate: Tổng tiền = Đơn giá × Số lượng mới
- [x] Success: Update table, toast "Đã cập nhật đơn hàng"

### US-8: Nhận mẫu về từ xưởng

**Là** Nhân viên  
**Tôi muốn** xác nhận đã nhận mẫu từ xưởng  
**Để** tracking trạng thái đơn hàng
...
- [x] Success: Button biến mất, toast "Đã xác nhận nhận mẫu", table refresh
...
- [x] Success: Xóa khỏi table, toast "Đã xóa đơn hàng"
...
- [x] Components (`src/features/labo-orders/components/`):
  - [x] `LaboOrderStatistics.tsx` - 2 KPI cards (Gửi đi + Nhận về)
