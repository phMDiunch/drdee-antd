# 🔄 Phase 0.1.3: ConsultedService Refactor for Deal-Centric Tracking

> **Date**: 2025-12-15  
> **Status**: 📝 PLANNING - Comprehensive refactor for deal-centric CRM  
...
Refactor **ConsultedService feature** (Sales Opportunity/Deal) để hỗ trợ:

1. ✅ Deal-level source tracking (dynamic campaign attribution)
2. ✅ 2-Phase Pipeline stage management:
   - **Lead Pipeline** (Sale Online): NEW → CONTACTING → SCHEDULED → ARRIVED
   - **Opportunity Pipeline** (Sale Offline): ARRIVED → CONSULTING → QUOTED → DEPOSIT → TREATING
   - **End States**: LOST (có thể xảy ra ở bất kỳ stage nào trong cả 2 phases)
3. ✅ Follow-up scheduling (priority, nextFollowUpDate)
4. ✅ Multiple sales roles (saleOnlineId for telesale, consultingSaleId for offline)
5. ✅ Lost deal tracking (lostReason, lostDate)

**Business Context:**

- ConsultedService = Sales Opportunity/Deal (không phải dịch vụ đã thực hiện)
- Mỗi deal có source riêng (khác với Customer.source)
- Deal source tracking cho phép tính ROI theo campaign cụ thể
- Pipeline stage cho phép visualize deal flow qua Kanban board
...
| `stage`            | String    | 2-Phase Pipeline stage: NEW/CONTACTING/SCHEDULED/ARRIVED (Lead) → CONSULTING/QUOTED/DEPOSIT/TREATING (Opportunity) → LOST (End state) |
...
| `lostReason`       | String?   | Reason if stage = LOST (có thể xảy ra ở bất kỳ phase nào)                                                                             |
...
- **Initial Stage Logic** (depends on customer type):
  - If `customer.type = LEAD` → `stage = NEW` (Lead Pipeline - need to contact)
  - If `customer.type = CUSTOMER` → `stage = ARRIVED` (Opportunity Pipeline - already at clinic)
- **2-Phase Pipeline:**
  - **Phase 1 (Lead)**: NEW → CONTACTING → SCHEDULED → ARRIVED (Sale Online target)
  - **Phase 2 (Opportunity)**: ARRIVED → CONSULTING → QUOTED → DEPOSIT → TREATING (Sale Offline/Doctor)
  - **End States**: LOST (có thể xảy ra ở bất kỳ stage nào), TREATING (thành công cuối cùng)
- `stage` determines active/completed:
  - **Active deals** = NOT IN ('LOST', 'TREATING') - bao gồm DEPOSIT vì có thể hủy
  - **Won deals** = 'TREATING' only - chỉ khi khách đã bắt đầu điều trị
  - **Lost deals** = 'LOST'
- **Important**: DEPOSIT có thể chuyển sang LOST (khách cọc nhưng sau đó hủy) hoặc TREATING (bắt đầu điều trị)
...
```typescript
// ⭐ NEW: Deal source (required, different from customer source)
source: z.string().trim().min(1, "Vui lòng chọn nguồn deal"),
sourceNotes: z.string().trim().optional().nullable(),

// ⭐ NEW: 2-Phase Pipeline stage
stage: z.enum([
  ...
  // End state (có thể xảy ra ở bất kỳ phase nào)
  "LOST"
]),
...
// ⭐ NEW: Multiple sales roles
saleOnlineId: z.string().uuid().optional().nullable(), // Telesale

// ⭐ NEW: Lost tracking (for LOST stage - có thể xảy ra ở cả 2 phases)
...
export const ConsultedServiceCreateRequestSchema = z.object({
  ...
  dentalServiceId: z.string().uuid("Dịch vụ không hợp lệ"),
  source: z.string().trim().min(1, "Vui lòng chọn nguồn deal"), // ⭐ REQUIRED
  stage: z
    .enum([
      "NEW",
      "CONTACTING",
    ...
    .optional(), // ⭐ Stage depends on customer.type: LEAD → NEW, CUSTOMER → ARRIVED
...
export const ConsultedServiceResponseSchema = z.object({
  ...
  // ⭐ Nested relations (không flatten)
  saleOnline: z.object({
    id: z.string(),
    fullName: z.string()
  }).nullable(),
...
async create(data: Prisma.ConsultedServiceCreateInput) {
  // ⭐ Get customer to determine initial stage
...
async list(params: {
  clinicId?: string;
  customerId?: string;
  source?: string; // ⭐ NEW filter
  stage?: string; // ⭐ NEW filter
  saleOnlineId?: string; // ⭐ NEW filter
  dateFrom?: Date;
  dateTo?: Date;
  skip?: number;
  take?: number;
  ...
  const where: Prisma.ConsultedServiceWhereInput = {
    ...
    source: params.source, // ⭐ Filter by deal source
    stage: params.stage, // ⭐ Filter by pipeline stage
    saleOnlineId: params.saleOnlineId, // ⭐ Filter by telesale
    createdAt: {
      gte: params.dateFrom,
      lte: params.dateTo,
    },
  ...
  return prisma.consultedService.findMany({
    ...
    include: {
      ...
      consultingDoctor: true,
      treatingDoctor: true,
      saleOnline: true, // ⭐ Include telesale
    },
...
async findDealsForFollowUp(params: {
  ...
  return prisma.consultedService.findMany({
    where: {
      stage: { notIn: ["LOST", "TREATING"] }, // ⭐ Active deals (bao gồm DEPOSIT vì có thể hủy)
      nextFollowUpDate: {
        gte: startOfDay,
        lte: endOfDay,
      },
...
async update(id: string, data: Prisma.ConsultedServiceUpdateInput) {
  // If stage changes to LOST, set lostDate
  if (data.stage === "LOST" && !data.lostDate) {
    data.lostDate = new Date();

  // If stage changes to DEPOSIT, set wonDate (optional, for tracking)
  if (data.stage === "DEPOSIT" && !data.wonDate) {
...
async create(currentUser: UserCore, body: unknown) {
  ...
  if (currentUser.role !== "admin" && data.clinicId !== currentUser.clinicId) {
    throw new ServiceError("CLINIC_MISMATCH", "Không có quyền tạo deal cho chi nhánh này", 403);
  }

  // ⭐ Ensure source is provided
  if (!data.source) {
    throw new ServiceError("VALIDATION_ERROR", "Nguồn deal là bắt buộc", 400);
  }

  // ⭐ Stage will be set by repo based on customer.type:
  // - If customer.type = LEAD → stage = NEW (Lead Pipeline)
  // - If customer.type = CUSTOMER → stage = ARRIVED (Opportunity Pipeline)
...
async update(currentUser: UserCore, id: string, body: unknown) {
  ...
  if (!existing) {
    throw new ServiceError("NOT_FOUND", "Không tìm thấy deal", 404);
  }

  // Validate clinic access
  if (currentUser.role !== "admin" && existing.clinicId !== currentUser.clinicId) {
    throw new ServiceError("CLINIC_MISMATCH", "Không có quyền cập nhật deal này", 403);
  }

// ⭐ Handle stage transition to LOST
  if (data.stage === "LOST") {
    if (!data.lostReason) {
      throw new ServiceError("VALIDATION_ERROR", "Vui lòng nhập lý do thất bại", 400);
    }
    data.lostDate = new Date();
  }

  // ⭐ Handle stage transition to TREATING (won deal - thành công cuối cùng)
  if (data.stage === "TREATING") {
    data.wonDate = new Date();
  }

  // DEPOSIT không set wonDate vì có thể hủy
}
```
...
async updateStage(currentUser: UserCore, id: string, body: unknown) {
  ...
  const updated = await consultedServiceRepo.update(id, {
    ...
    wonDate: data.stage === "TREATING" ? new Date() : null, // Chỉ TREATING mới là won
...
// ⭐ NEW: Update stage (for Kanban drag & drop)
...
// ⭐ NEW: Update follow-up (for follow-up scheduling)
...
// ⭐ EXISTING: Update with new fields
...
export async function GET(req: Request) {
  try {
    ...
    const params = {
      ...
      source: searchParams.get("source") || undefined, // ⭐ NEW
      stage: searchParams.get("stage") || undefined, // ⭐ NEW
      saleOnlineId: searchParams.get("saleOnlineId") || undefined, // ⭐ NEW
...
<Row gutter={12}>
  <Col xs={24} lg={12}>
    <Form.Item
      label="Nguồn deal"
      name="source"
      rules={[{ required: true, message: "Vui lòng chọn nguồn deal" }]}
      tooltip="Nguồn của deal này (campaign cụ thể)"
    >
      <Select
        placeholder="Chọn nguồn deal"
  ...
  <Col xs={24} lg={12}>
    <Form.Item label="Ghi chú nguồn" name="sourceNotes">
      <Input.TextArea
        placeholder="VD: Facebook Ads - Campaign Tháng 12"
        rows={1}
      />
    </Form.Item>
  </Col>
...
<Row gutter={12}>
  <Col xs={24} lg={8}>
    ...
    >
      <Select placeholder="Chọn trạng thái" options={PIPELINE_STAGE_OPTIONS} />
    </Form.Item>
  </Col>
  <Col xs={24} lg={8}>
    <Form.Item label="Độ ưu tiên follow-up" name="followUpPriority">
      <Select
  ...
  <Col xs={24} lg={8}>
    <Form.Item label="Ngày follow-up tiếp theo" name="nextFollowUpDate">
      <DatePicker
        style={{ width: "100%" }}
        format="DD/MM/YYYY"
    ...
    <Form.Item noStyle shouldUpdate={(prev, curr) => prev.stage !== curr.stage}>
      {({ getFieldValue }) => {
        ...
        return stage === "LOST" ? (
          ...
          >
            <Input.TextArea placeholder="Nhập lý do thất bại" rows={1} />
          </Form.Item>
        ) : null;
      }}
    </Form.Item>
...
// ⭐ Query Keys (GUIDELINES requirement)
...
// ⭐ 2-Phase Real-World Pipeline
// Phase 1: Lead Pipeline (Sale Online) - Mục tiêu: Kéo khách đến phòng khám
// Phase 2: Opportunity Pipeline (Sale Offline/Doctor) - Mục tiêu: Chốt tiền và Điều trị

export const PIPELINE_STAGE_OPTIONS = [
  // === PHASE 1: LEAD PIPELINE (Sale Online) ===
  { label: "Mới", value: "NEW", color: "blue", phase: "lead" },
  { label: "Đang liên hệ", value: "CONTACTING", color: "cyan", phase: "lead" },
  {
    label: "Đã đặt lịch",
    value: "SCHEDULED",
    color: "geekblue",
  ...
  { label: "Đã đến", value: "ARRIVED", color: "purple", phase: "lead" }, // KPI Sale Online
  ...
  { label: "Đã báo giá", value: "QUOTED", color: "gold", phase: "opportunity" },
  { label: "Đã cọc", value: "DEPOSIT", color: "green", phase: "opportunity" }, // WIN - End state
  {
    label: "Đang điều trị",
    value: "TREATING",
    color: "lime",
  ...
  // === END STATE (có thể xảy ra ở bất kỳ phase nào) ===
  { label: "Thất bại", value: "LOST", color: "red", phase: "end" },
];

// ⭐ Reuse CUSTOMER_SOURCES from Customer feature
// Deal source same as customer source (giữ consistency)
...
export const FOLLOW_UP_PRIORITY_OPTIONS = [
  ...
  { label: "Trung bình", value: "MEDIUM", color: "orange" },
  { label: "Thấp", value: "LOW", color: "blue" },
];

// ⭐ Messages (GUIDELINES requirement)
export const CONSULTED_SERVICE_MESSAGES = {
  CREATE_SUCCESS: "Tạo deal thành công",
  UPDATE_SUCCESS: "Cập nhật deal thành công",
  DELETE_SUCCESS: "Xóa deal thành công",
  UPDATE_STAGE_SUCCESS: "Cập nhật trạng thái thành công",
  UPDATE_FOLLOW_UP_SUCCESS: "Cập nhật lịch follow-up thành công",
} as const;
```
...
const initialValues = {
  ...
  sourceNotes: "",
  stage: undefined, // ⭐ Will be set by backend based on customer.type (LEAD→NEW, CUSTOMER→ARRIVED)
...
```tsx
{
  ...
  render: (_, record) => {
    ...
    // DEPOSIT vẫn hiện follow-up vì có thể hủy

    if (!record.nextFollowUpDate) {
      return <Text type="secondary">Chưa đặt lịch</Text>;
    }
...
```tsx
{
  ...
  key: "saleOnline",
  width: 120,
  render: (_, record) => record.saleOnline?.fullName || "—",
}
```
...
```tsx
<Form.Item label="Trạng thái" name="stage">
  <Select
...
```tsx
<Form.Item label="Nguồn deal" name="source">
  <Select placeholder="Tất cả nguồn" options={DEAL_SOURCE_OPTIONS} allowClear />
</Form.Item>
```
...
<Descriptions column={2} bordered>
  <Descriptions.Item label="Nguồn deal">{data.source || "—"}</Descriptions.Item>
  <Descriptions.Item label="Ghi chú nguồn">
    {data.sourceNotes || "—"}
  </Descriptions.Item>
  <Descriptions.Item label="Nguồn khách hàng" span={2}>
    <Text type="secondary">
      {data.customer.source || "—"} (nguồn gốc ban đầu)
    </Text>
  </Descriptions.Item>
</Descriptions>
```
...
<Card title="Follow-up" style={{ marginTop: 16 }}>
  <Descriptions column={2} bordered>
    <Descriptions.Item label="Ngày follow-up tiếp theo">
    ...
    <Descriptions.Item label="Sale Online">
      {data.saleOnline?.fullName || "—"}
    </Descriptions.Item>
    <Descriptions.Item label="Sale Offline">
      {data.consultingSale?.fullName || "—"}
    </Descriptions.Item>
  </Descriptions>
...
{
  data.stage === "FAILED" && (
    <Card title="Thông tin thất bại" style={{ marginTop: 16 }}>
      <Descriptions column={1} bordered>
        <Descriptions.Item label="Lý do">
          {data.failedReason || "—"}
        </Descriptions.Item>
        <Descriptions.Item label="Ngày thất bại">
          {data.failedDate ? dayjs(data.failedDate).format("DD/MM/YYYY") : "—"}
        </Descriptions.Item>
      </Descriptions>
    </Card>
  );
...
}
{
  ["DEPOSIT", "TREATING"].includes(data.stage) && (
    <Card title="Thông tin thành công" style={{ marginTop: 16 }}>
      <Descriptions column={1} bordered>
        <Descriptions.Item label="Trạng thái">
          <Tag color="green">
            {data.stage === "DEPOSIT" ? "Đã cọc" : "Đang điều trị"}
          </Tag>
        </Descriptions.Item>
        <Descriptions.Item label="Ngày thành công">
          {data.wonDate ? dayjs(data.wonDate).format("DD/MM/YYYY") : "—"}
        </Descriptions.Item>
      </Descriptions>
    </Card>
  );
...
export function UpdateFollowUpModal({
  ...
  return (
    ...
    >
      ...
      >
        <Form.Item label="Độ ưu tiên" name="followUpPriority">
          <Select
            placeholder="Chọn độ ưu tiên"
...
export function StageUpdateModal({
  ...
  return (
    <Modal
      open={open}
      title="Cập nhật trạng thái deal"
      onCancel={onCancel}
      onOk={() => form.submit()}
      confirmLoading={mutation.isPending}
    >
          ...
          {({ getFieldValue }) => {
            ...
            return stage === "LOST" ? (
              <Form.Item
                ...
                rules={[{ required: true, message: "Vui lòng nhập lý do" }]}
              >
                <Input.TextArea placeholder="Nhập lý do thất bại" rows={3} />
              </Form.Item>
            ) : null;
          }}
        </Form.Item>
...
export function useUpdateStage(consultedServiceId: string) {
  ...
  const notify = useNotify(); // ⭐ GUIDELINES: ALWAYS use useNotify()
...
**Purpose:** Màn hình chính cho sales team xem danh sách deals cần follow-up hôm nay/ngày được chọn.

**Use Case:** Sales mở app mỗi sáng → thấy ngay danh sách khách cần gọi → click để call trực tiếp.
...
- Filter by date (default: hôm nay)
- Filter by sale (manager xem tất cả, sales chỉ xem của mình)
- Display deals sorted by priority (HIGH → MEDIUM → LOW)
...
export function FollowUpDashboardView() {
  ...
  // ⭐ Fetch danh sách sales (cho manager)
  ...
  return (
    <div style={{ padding: "24px" }}>
      {/* Header */}
      <Title level={2}>📞 Follow-up Dashboard</Title>
      <Text type="secondary">Danh sách deals cần follow-up hôm nay</Text>
          ...
          {user.role !== "employee" && (
            <div>
              ...
              <Select
                ...
                options={[
                  { label: "Tất cả sales", value: undefined },
                  ...(salesOptions || []),
                ]}
                style={{ width: 200 }}
                allowClear
      ...
      <Card>
        {isLoading ? (
                  ...
                  <List.Item.Meta
                    title={
                      <Space>
                        ...
                        {isToday && <Tag color="orange">Hôm nay</Tag>}
                      </Space>
                    }
                    description={
                      <Space direction="vertical" size={4}>
                        <Text>
                          <strong>Dịch vụ:</strong> {deal.dentalService.name}
                        </Text>
                        <Text>
                          <strong>SĐT:</strong>{" "}
                          {deal.customer.phone || "Chưa có"}
                        </Text>
                        <Space>
                          <Text>
                            <CalendarOutlined />{" "}
          ...
          <Empty
            description={`Không có deal nào cần follow-up vào ${dayjs(
              selectedDate
            ).format("DD/MM/YYYY")}`}
          />
        )}
...
1. Sales login → Auto navigate to Follow-up Dashboard
2. Thấy 5 deals cần gọi hôm nay
3. Click "Gọi" → Phone app mở với số điện thoại
4. Sau khi gọi xong → Update stage hoặc reschedule follow-up

**Manager user flow:**

1. Manager mở Follow-up Dashboard
2. Chọn "Tất cả sales" → Thấy tổng quan
3. Chọn sale cụ thể → Kiểm tra công việc của sale đó
4. Thấy deals quá hạn → Nhắc nhở sale

---

## 🎯 2-PHASE KANBAN VIEWS

### Concept: Online View vs Offline View

**Purpose:** Tách riêng 2 màn hình Kanban để phù hợp với workflow thực tế:

- **Online View**: Sale Online (telesale) chỉ thấy các cột Lead Pipeline
- **Offline View**: Sale Offline + Bác sĩ thấy các cột Opportunity Pipeline
...
**Goal:** Kéo khách đến phòng khám (target = ARRIVED)

**Kanban Columns:**

| Column       | Stage      | Description                                                  | Actions                               |
| ------------ | ---------- | ------------------------------------------------------------ | ------------------------------------- |
| Mới          | NEW        | Data mới đổ về, chưa ai đụng                                 | → Kéo sang CONTACTING khi bắt đầu gọi |
| Đang liên hệ | CONTACTING | Đã gọi/nhắn nhưng chưa chốt lịch                             | → SCHEDULED (chốt lịch) hoặc LOST     |
| Đã đặt lịch  | SCHEDULED  | Khách đồng ý đến khám vào giờ cụ thể                         | → ARRIVED (khách đến) hoặc LOST       |
| Đã đến       | ARRIVED    | ✅ **KPI Sale Online** - Khách đã có mặt                     | → Chuyển sang Opportunity Pipeline    |
| Thất bại     | LOST       | Sai số, dập máy, hủy lịch (có thể xảy ra ở bất kỳ stage nào) | ❌ End state                          |

**Route:** `/leads` hoặc `/online-pipeline`
...
**Audience:** Sale Offline + Bác sĩ + Lễ tân

**Goal:** Chốt tiền và điều trị (target = DEPOSIT → TREATING)

**Kanban Columns:**

| Column        | Stage      | Description                                                     | Actions                         |
| ------------- | ---------- | --------------------------------------------------------------- | ------------------------------- |
| Hàng đợi      | ARRIVED    | Khách đã đến, chờ gặp bác sĩ                                    | → CONSULTING (bắt đầu khám)     |
| Đang tư vấn   | CONSULTING | Bác sĩ đang khám, chụp X-quang, lên phác đồ                     | → QUOTED (báo giá) hoặc LOST    |
| Đã báo giá    | QUOTED     | Đã đưa ra con số, khách đang cân nhắc                           | → DEPOSIT (chốt tiền) hoặc LOST |
| Đã cọc ✅     | DEPOSIT    | **WIN** - Khách đã xuống tiền cọc                               | → TREATING (bắt đầu điều trị)   |
| Đang điều trị | TREATING   | Quá trình điều trị đang diễn ra                                 | Post-WIN state                  |
| Thất bại      | LOST       | Khách không đồng ý, hủy deal (có thể xảy ra ở bất kỳ stage nào) | ❌ End state                    |

**Route:** `/opportunities` hoặc `/offline-pipeline`
  ...
  return (
    <div>
      ...
      <Row gutter={16}>
        ...
        <Col span={6}>
          <Card>
            <Statistic title="Đang điều trị" value={treatingCount} />
          </Card>
        </Col>
        <Col span={6}>
          <Card>
            <Statistic title="Đã báo giá" value={quotedCount} />
          </Card>
        </Col>
        <Col span={6}>
          <Card>
...
| ------------ | ------------- | ------------------------------------------- |
| Sale Online  | Lead Pipeline | NEW → CONTACTING → SCHEDULED → ARRIVED/LOST |
| Sale Offline | Opportunity   | ARRIVED → CONSULTING → QUOTED → DEPOSIT     |
| Doctor       | Opportunity   | CONSULTING → QUOTED                         |
...
export function canUpdateStage(
  ...
  if (user.role === "sale_offline") {
    const opportunityStages = [
      ...
      "DEPOSIT",
      "TREATING",
      "LOST", // Offline cũng có thể mark deal as LOST
    ];
    return (
...
async findByStages(params: {
  stages: string[]; // ⭐ Filter by specific stages only
  clinicId?: string;
  saleOnlineId?: string;
  ...
  return prisma.consultedService.findMany({
    where: {
      stage: { in: params.stages }, // ⭐ Only fetch specified stages
      clinicId: params.clinicId,
...
export async function GET(req: Request) {
  try {
    ...
    // ⭐ Accept stages as comma-separated list
...
- ✅ Create deal with source (required)
- ✅ Create deal with default stage = "NEW"
- ✅ Update stage to LOST (requires lostReason)
- ✅ Update stage to TREATING (sets wonDate automatically)
- ✅ DEPOSIT does not set wonDate (can still be cancelled)
- ✅ Update follow-up fields
- ✅ Filter deals by stage (2-phase pipeline)
- ✅ Filter deals by source
- ✅ Get active deals for follow-up (stage NOT IN ['LOST', 'TREATING']) - includes DEPOSIT
- ✅ Filter deals by stages array (for Online/Offline views)
- ✅ LOST can happen in both Lead and Opportunity pipelines
- ✅ DEPOSIT can transition to LOST (customer cancels after deposit)
...
- ✅ ConsultedServiceFormModal shows all new fields
- ✅ Source field is required
- ✅ Lost reason required when stage = LOST
- ✅ ConsultedServiceTable shows new columns
- ✅ Follow-up column shows overdue/today badges (for active deals including DEPOSIT)
- ✅ UpdateFollowUpModal updates fields
- ✅ StageUpdateModal updates stage (with lostReason validation)
- ✅ LeadPipelineKanbanView shows lead stages + LOST (NEW → ARRIVED/LOST)
- ✅ OpportunityPipelineKanbanView shows opportunity stages + LOST (ARRIVED → TREATING/LOST)
- ✅ Role-based permissions: Sale Online can mark deal as LOST
- ✅ Role-based permissions: Sale Offline can also mark deal as LOST
- ✅ LOST stage can be set from any active stage in both pipelines
- ✅ DEPOSIT stage can transition to TREATING (start treatment) or LOST (cancel)
- ✅ Only TREATING shows "Thành công" section in DetailView
- ✅ DEPOSIT shows "Đặt cọc" section with warning about possible cancellation
...
- [ ] useUpdateStage hook (với useNotify + query keys)
- [ ] useUpdateFollowUp hook (với useNotify + query keys)
- [ ] useDealsForFollowUp hook (với caching strategy)
...
**2-Phase Kanban Views:** ⏳ TODO

- [ ] LeadPipelineKanbanView (Online View: NEW → ARRIVED/FAILED)
- [ ] OpportunityPipelineKanbanView (Offline View: ARRIVED → TREATING)
...
- **NEW**: 2-Phase Pipeline stages (9 stages total):
  ...
  - **End State**: LOST (có thể xảy ra ở bất kỳ stage nào trong cả 2 phases)
...
- **NEW**: 2 Kanban Views:
  - **LeadPipelineKanbanView**: Sale Online view (NEW → ARRIVED/FAILED)
  - **OpportunityPipelineKanbanView**: Sale Offline view (ARRIVED → TREATING)
...
- **2-Phase Pipeline stage management**:
  - **Lead Pipeline** (Sale Online): NEW → CONTACTING → SCHEDULED → ARRIVED
  - **Opportunity Pipeline** (Sale Offline/Doctor): ARRIVED → CONSULTING → QUOTED → DEPOSIT → TREATING
  - **End States**: LOST (có thể xảy ra ở bất kỳ stage nào), TREATING (thành công cuối cùng)
- Follow-up scheduling (priority + date)
- Active vs completed deals:
  - **Active** = NOT IN ['LOST', 'TREATING'] - bao gồm DEPOSIT vì có thể hủy
  - **Won** = 'TREATING' only - chỉ khi đã bắt đầu điều trị
  - **Lost** = 'LOST'
- Lost deal tracking (reason + date) - có thể xảy ra ở cả 2 phases
- Won deal tracking (wonDate when TREATING, not DEPOSIT)
- **DEPOSIT logic**: Đã cọc nhưng vẫn có thể hủy → chuyển sang TREATING (thành công) hoặc LOST (hủy)
- Multiple sales roles (saleOnlineId for telesale, consultingSaleId for offline)
- **Role-based Kanban views** (Online vs Offline) - cả 2 view đều có thể mark deal as LOST
