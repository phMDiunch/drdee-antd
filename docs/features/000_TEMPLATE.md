...
│
└── shared/validation/
    └── [feature].schema.ts             # ✅ Zod schemas & types (z.infer)
```
...
### 📥 **Request/Response (từ shared/validation):**
...
// Types sử dụng trong component/hook
import type { z } from "zod";
type CreateRequest = z.infer<typeof Create[Feature]RequestSchema>;
type UpdateRequest = z.infer<typeof Update[Feature]RequestSchema>;
```
...
// Sử dụng
import type { z } from "zod";
type [Feature]Response = z.infer<typeof [Feature]ResponseSchema>;
```
...
// Hooks (barrel export từ hooks/index.ts)
