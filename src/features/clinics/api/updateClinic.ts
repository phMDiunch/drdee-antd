...
export async function updateClinicApi(id: string, payload: Omit<UpdateClinicRequest, "id">) {
  const body = UpdateClinicRequestSchema.parse({ ...payload, id });
  ...
  if (!parsed.success) throw new Error("Phản hồi cập nhật phòng khám không hợp lệ.");
  return parsed.data;
}
