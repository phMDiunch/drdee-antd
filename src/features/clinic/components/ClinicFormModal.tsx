// src/features/clinic/components/ClinicFormModal.tsx
"use client";
...
import { ClinicResponse, CreateClinicRequest, UpdateClinicRequest } from "@/features/clinic/types";
import { CreateClinicRequestSchema, UpdateClinicRequestSchema } from "@/shared/validation/clinic.schema";
...
export default function ClinicFormModal({ open, mode, isAdmin, initial, confirmLoading, onCancel, onSubmit }: Props) {
  // Chọn schema theo mode
  const schema = useMemo(() => (mode === "create" ? CreateClinicRequestSchema : UpdateClinicRequestSchema), [mode]);
              ...
              render={({ field, fieldState }) => (
                ...
                >
                  <ColorPicker
                    ...
                    onChange={(c) => field.onChange(c.toHexString().toLowerCase())}
                  />
                </Form.Item>
              )}
            />
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Form.Item
                  ...
                  help={fieldState.error?.message}
                >
                  <Input {...field} value={field.value ?? ""} placeholder="0987654321" />
                </Form.Item>
              )}
            />
          </Col>
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Form.Item
                  ...
                  help={fieldState.error?.message}
                >
                  <Input {...field} value={field.value ?? ""} placeholder="clinic@example.com" />
                </Form.Item>
              )}
            />
          </Col>
          ...
          <Row gutter={12}>
            <Col xs={24} md={12}>
              <Form.Item label="Ngày tạo">
                <Input disabled value={dayjs(initial.createdAt).format("DD/MM/YYYY HH:mm")} />
              </Form.Item>
            </Col>
            <Col xs={24} md={12}>
              <Form.Item label="Cập nhật">
                <Input disabled value={dayjs(initial.updatedAt).format("DD/MM/YYYY HH:mm")} />
              </Form.Item>
            </Col>
          </Row>
        )}
