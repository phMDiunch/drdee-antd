...
import type { ServiceGroupRevenueData } from "@/shared/validation/revenue-report.schema";

interface ServiceGroupRevenueTableProps {
  data: ServiceGroupRevenueData[];
  loading?: boolean;
  onRowClick?: (record: ServiceGroupRevenueData) => void;
}

export default function ServiceGroupRevenueTable({
  data,
  loading,
  onRowClick,
}: ServiceGroupRevenueTableProps) {
  ...
  const columns = useMemo<ColumnsType<ServiceGroupRevenueData>>(
    () => [
      {
        title: "Nhóm dịch vụ",
