...
export async function exportConsultedServicesToExcel(
  ...
  worksheet.columns = [
    ...
    { header: "Sale online", key: "saleOnline", width: 20 },
    { header: "Bác sĩ tư vấn", key: "consultingDoctor", width: 20 },
    { header: "NV tư vấn", key: "sale", width: 20 },
    { header: "Bác sĩ điều trị", key: "treatingDoctor", width: 20 },
  ...
  data.forEach((item, index) => {
    worksheet.addRow({
      ...
      debt: item.debt,
      saleOnline: item.saleOnline?.fullName || "",
      consultingDoctor: item.consultingDoctor?.fullName || "",
      sale: item.consultingSale?.fullName || "",
      treatingDoctor: item.treatingDoctor?.fullName || "",
